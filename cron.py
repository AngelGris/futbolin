...
for round in rounds:
    # Play matches
    rounds_count += 1
    category = db_connection.query('SELECT `category_id` FROM `tournament_rounds` WHERE `id` = ' + str(round['id']) + ' LIMIT 1', 1)
    ...
    for match in matches:
        ...
        db_connection.query('UPDATE `tournament_positions` SET `points` = `points` + ' + str((results[0][0] * 3) + results[0][1]) + ', `played` = `played` + 1, `won` = `won` + ' + str(results[0][0]) + ', `tied` = `tied` + ' + str(results[0][1]) + ', `lost` = `lost` + ' + str(results[0][2]) + ', `goals_favor` = `goals_favor` + ' + str(match['local_goals']) + ', `goals_against` = `goals_against` + ' + str(match['visit_goals']) + ', `goals_difference` = `goals_difference` + ' + str(match['local_goals'] - match['visit_goals']) + ' WHERE `category_id` = ' + str(category['category_id']) + ' AND `team_id` = ' + str(match['local_id']) + ' LIMIT 1')
        db_connection.query('UPDATE `tournament_positions` SET `points` = `points` + ' + str((results[1][0] * 3) + results[1][1]) + ', `played` = `played` + 1, `won` = `won` + ' + str(results[1][0]) + ', `tied` = `tied` + ' + str(results[1][1]) + ', `lost` = `lost` + ' + str(results[1][2]) + ', `goals_favor` = `goals_favor` + ' + str(match['visit_goals']) + ', `goals_against` = `goals_against` + ' + str(match['local_goals']) + ', `goals_difference` = `goals_difference` + ' + str(match['visit_goals'] - match['local_goals']) + ' WHERE `category_id` = ' + str(category['category_id']) + ' AND `team_id` = ' + str(match['visit_id']) + ' LIMIT 1')

    # Update positions
    teams = db_connection.query('SELECT `team_id` FROM `tournament_positions` WHERE `category_id` = ' + str(category['category_id']) + ' ORDER BY `points` DESC, `goals_difference` DESC, `goals_favor` DESC LIMIT 20;', 20);
    position = 0
    for team in teams:
        position += 1
        db_connection.query('UPDATE `tournament_positions` SET `position` = ' + str(position) + ' WHERE `category_id` = ' + str(category['category_id']) + ' AND `team_id` = ' + str(team['team_id']) + ' LIMIT 1;')
